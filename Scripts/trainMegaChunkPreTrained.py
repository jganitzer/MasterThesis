...
from models.megaencoder import MegaChunkRegressor_MLP_cat_PreTrained_CLS
from utils.torchDataloader import CatMaskLoaderCLS2
...
preTrainedPath = "/data/_/pretrained/" #6 Layer
...
#catLoader = CatMaskLoader(dataRoot + heritability, X_path, y_path, wandbDir=wandbDir, batch_size = batch_size, val_size = val_size, paddedLength = paddedLength)
#catLoader = CatMaskLoaderCLS(dataRoot + heritability, X_path, y_path, wandbDir=wandbDir, batch_size = batch_size, val_size = val_size, paddedLength = paddedLength)
catLoader = CatMaskLoaderCLS2(file_path=file_path, label_path=label_path, batch_size = batch_size, val_size = val_size, paddedLength = paddedLength)
...
model = MegaChunkRegressor_MLP_cat_PreTrained_CLS(input_dim=input_dim, num_layers=num_layers, num_heads=num_heads, max_pos=max_pos, hidden1 = hidden1, hidden2 = hidden2, hidden3 = hidden3, num_features = num_features, chunk_size=chunk_size, preTrainedPath = preTrainedPath).to(device)

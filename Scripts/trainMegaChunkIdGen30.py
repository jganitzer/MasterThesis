...
from utils.torchDataloader import CatMaskLoaderfine
...
catLoader = CatMaskLoaderfine(dataRoot + heritability, trainPath, testPath, wandbDir=wandbDir, batch_size = batch_size, paddedLength = paddedLength)
...
def train(model: nn.Module) -> float:
    ...
    for batch, (data, targets, attention_mask_train) in enumerate(train_loader):

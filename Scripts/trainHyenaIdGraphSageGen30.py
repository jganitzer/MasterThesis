...
from models.megagraphsage import HyenaGraphSage_3MLP_cat_Reduced
...
hyenaPreTrainedPath = "/data/checkpoints/_/" + "best_model_state_dict.pth"

sagePreTrainedPath = "/data/checkpoints/_/" + "best_model.pth"
...
model = HyenaGraphSage_3MLP_cat_Reduced(input_dim=input_dim, in_channels = data.x_oneHot.size(1)* data.x_oneHot.size(2), hidden_channels = embed_dim, 
...
def train(model: nn.Module) -> float:
    ...
    for batch in train_loader:
        with torch.cuda.amp.autocast():
            ...
            y_hat = model(batch.x_oneHot, batch.edge_index, batch.x[:batch.batch_size], batch.batch_size)

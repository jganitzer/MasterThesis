...
from utils.torchDataloader import OneHotMaskLoader
...
config = {
    ...
    "additional_settings": "BatchNorm, float32, linarReductionLayer"  
}

run = wandb.init(project=wandb_project_name, name=wandb_run_name, entity=entity, config=config)

oneHotLoader = OneHotMaskLoader(dataRoot + heritability, X_path, y_path, wandbDir=wandbDir, batch_size = batch_size, val_size = val_size, paddedLength = paddedLength)

...
from models.megaencoder import MegaChunkRegressor_MLP_cat_CLS
from utils.torchDataloader import CatLoaderCLSTokenizerPad
...
catLoader = CatLoaderCLSTokenizerPad(file_path=file_path, label_path=label_path, batch_size = batch_size, val_size = 0.2, paddedLength = paddedLength)

train_loader, val_loader = catLoader.getLoaders()

num_features = paddedLength

model = MegaChunkRegressor_MLP_cat_CLS(input_dim=input_dim, num_layers=num_layers, num_heads=num_heads, max_pos=max_pos, hidden1 = hidden1, hidden2 = hidden2, hidden3 = hidden3, num_features = num_features, chunk_size=chunk_size).to(device)
...
def train(model: nn.Module) -> float:
    ...
    for _, (data, targets, attention_mask_train) in enumerate(train_loader):
...
def evaluate(model: nn.Module, eval_loader: DataLoader) -> float:
    ...
    with torch.no_grad():
        for data, targets, attention_mask_val in eval_loader:
            with torch.cuda.amp.autocast():
                data = data.to(device).type(dtype1)
                targets = targets.squeeze().to(device).type(dtype2)

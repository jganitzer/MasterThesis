...
class MLM(nn.Module):
    ...
    def forward(self, seq, **kwargs):
        ...
        # Generate attention mask based on masked_seq
        attention_mask = (~(masked_seq == self.pad_token_id) & ~(masked_seq == self.mask_token_id)).to(torch.long)

        out = self.transformer(masked_seq, attention_mask=attention_mask, **kwargs)

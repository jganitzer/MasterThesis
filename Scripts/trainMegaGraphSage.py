...
from models.megagraphsage import MegaGraphSage_3MLP_Reduced
...
num_features = data.x_pad.size(1)
#model = MegaGraphSage_3MLP(input_dim=input_dim, in_channels = data.x.size(1)* data.x.size(2), hidden_channels = embed_dim, 
#out_channels = 1,num_layers=num_layers, num_heads=num_heads, max_pos=max_pos, hidden1 = hidden1, hidden2 = hidden2, 
#hidden3 = hidden3, num_features = num_features, chunk_size=chunk_size).to(device)

model = MegaGraphSage_3MLP_Reduced(input_dim=input_dim, in_channels = data.x.size(1)* data.x.size(2), hidden_channels = embed_dim, 
out_channels = 1,num_layers=num_layers, num_heads=num_heads, max_pos=max_pos, hidden1 = hidden1, hidden2 = hidden2, 
hidden3 = hidden3, num_features = num_features, chunk_size=chunk_size).to(device)
...
def train(model: nn.Module) -> float:
    ...
    for batch in train_loader:
        with torch.cuda.amp.autocast():
            ...
            y_hat = model(batch.x, batch.edge_index, batch.x_pad[:batch.batch_size], batch.attention_mask[:batch.batch_size], batch.batch_size)
...
def evaluate(model: nn.Module) -> float:
    ...
    total_loss = 0.
    total_mae = 0.  # initialize mean absolute error
    total_r2 = 0.  # initialize R^2 score
    total_pearson = 0.  # initialize Pearson correlation
    total_pvalue = 0.  # initialize p-value
    total_examples = 0
...
def evaluate_gen30(model: nn.Module) -> float:
    ...
    val_loss_gen30, val_mae_gen30, val_rsquare_gen30, val_pearson_gen30, val_pvalue_gen30 = model.inference_gen30(subgraph_loader, data, batch_size, criterion, device)
        
    return val_loss_gen30, val_mae_gen30, val_rsquare_gen30, val_pearson_gen30, val_pvalue_gen30
...
for epoch in range(1, num_epochs + 1):
    ...
    val_loss_gen30, val_mae_gen30, val_rsquare_gen30, val_pearson_gen30, val_pvalue_gen30 = evaluate_gen30(model)

    elapsed = time.time() - epoch_start_time
    print('-' * 180)
    print(f'| end of epoch {epoch:3d} | time: {elapsed:5.2f}s | '
        f'train loss {train_loss:5.4f} | valid loss {val_loss_gen30:5.4f} | train MAE {train_mae:5.4f} | '
        f'valid MAE {val_mae_gen30:5.4f} | train R^2 {train_rsquare:5.4f} | valid R^2 {val_rsquare_gen30:5.4f} |'
        f'learning rate {scheduler.get_last_lr()[0]:5.7f} |'
        )

    print(f'| train pearson {train_pearson:5.4f} | valid pearson {val_pearson_gen30:5.4f} | train p-value {train_pvalue:5.4f} | valid p-value {val_pvalue_gen30:5.4f} |')
    ...
    wandb.log({"epoch": epoch, "val_loss": val_loss_gen30, "epoch": epoch})

    if val_loss_gen30 < best_val_loss:
        best_val_loss = val_loss_gen30
        best_epoch = epoch
        print("Best epoch %d" % best_epoch)
        checkpoint(model, checkpoint_path + "best_model.pth", epoch, val_loss_gen30, optimizer, scheduler)

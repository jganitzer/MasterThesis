...
from utils.torchDataloader import OneHotLoaderFine
...
trainPath = '/snp_pheno_BPpos_maf_new_one_hot.parquet'
testPath = '/snpTruePheno_gen30_BPpos_maf_new_one_hot.parquet'

trainPath = '/snp_maf_one_hot.parquet'
...
oneHotLoader = OneHotLoaderFine(dataRoot + heritability, trainPath, testPath, wandbDir=wandbDir, batch_size = batch_size)
...
if resume_training == True:
    ...
    # Set the learning rate of the optimizer to a non-zero value
...
def train(model: nn.Module) -> float:
    ...
    #for batch, (data, targets, _) in enumerate(train_loader):

...
from utils.torchDataloader import CatLoader
...
catLoader = CatLoader(dataRoot + heritability, X_path, y_path, wandbDir=wandbDir, batch_size = batch_size, val_size = val_size)
#num_features = 26284
...
num_features = train_loader.dataset.tensors[0].shape[1] #TODO: Uncomment when not in CLS mode
...
#model = HyenaRegressor_MLP_cat_CLS(input_dim=input_dim, num_layers=num_layers, num_heads=num_heads, max_pos=num_features, hidden1 = hidden1, hidden2 = hidden2, hidden3 = hidden3, num_features = num_features, order = order, activation = activation).to(device)

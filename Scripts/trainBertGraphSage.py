...
from utils.helper import checkpoint, load_checkpoint, checkpoint2
...
from models.megagraphsage import BertGraphSage_MLP
...
X_path = "/train/snp_X_train_one_hot.parquet"
y_path = "/train/snp_y_train_one_hot.parquet"
...
model = BertGraphSage_MLP(input_dim=input_dim, in_channels = data.x.size(1)* data.x.size(2), hidden_channels = embed_dim, 
out_channels = 1,num_layers=num_layers, num_heads=num_heads, max_pos=max_pos, hidden1 = hidden1, hidden2 = hidden2, 
hidden3 = hidden3, num_features = num_features).to(device)

...
class SequenceDecoderRegression(nn.Module):
    def __init__(self, d_model, d_output=1, l_output=None, use_lengths=False, mode="last"):
        super().__init__()
...
class HyenaDNAModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_layer: int, d_inner: int, vocab_size: int,
                 ...
                 resid_dropout: float = 0.0, embed_dropout: float = 0.1,
                 layer_norm_epsilon: float = 1e-5, initializer_cfg=None,residual_in_fp32=False,
                 pad_vocab_size_multiple: int = 1, use_head=False, n_classes: int = 1, l_output=None, mode='last', num_features = 26282, hidden1 = 1000, hidden2 = 500,
                 device=None, dtype=None, **kwargs) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        super().__init__()
        ...
        #if self.use_head:
            #self.head = SequenceDecoderRegression(d_model=d_model, d_output=n_classes, l_output=l_output, mode=mode)
    ...
    def forward(self, input_ids, position_ids=None, state=None): # state for the repo interface
        ...
        reduced_tensor = self.linear_layer(hidden_states).squeeze()

        logits = self.fc(reduced_tensor).squeeze()  # (batch_size, 1)

        return logits, reduced_tensor, hidden_states 
